...
//    printf("%s\n", dir_destn);
    ...
    char *dirs_new = (char*)malloc(strlen(dirs) + strlen(dir_destn) + 2);
    if(!dirs_new){
        fprintf(stderr, "sh: malloc failure\n");
        exit(EXIT_FAILURE);
    }
//    memcpy(dirs_new, dir_destn, len1);
//    memcpy(dirs_new, " i", len1 + 1);
//    memcpy(dirs_new, dirs, len2+1);
//    printf("%s\n", dirs_new);

    strcpy(dirs_new, dir_destn);
    strcat(dirs_new, " ");
    strcat(dirs_new, dirs);
    strcat(dirs_new, "\0");
